...
Class Actions extends DBConnection{
    ...
    function add_to_favorite(){
        ...
        $check = $this->query("SELECT count(book_id) as `count` FROM `favorite_list` where `book_id` = '{$book_id}' and `user_id` = '{$user_id}'")->fetchArray()['count'];
        if($check > 0){
            $resp['status'] ='failed';
            $resp['msg'] ='The SLR is already on your favorite list';
        }else{
            $sql = "INSERT INTO `favorite_list` (`book_id`,`user_id`)VALUES('{$book_id}','{$user_id}')";
            $save = $this->query($sql);
            if($save){
                $resp['status'] ='success';
                $count = $this->query("SELECT count(book_id) as total FROM `favorite_list` where `user_id` = '{$user_id}'")->fetchArray()['total'];
                $resp['favorite_count'] = $count;
            }else{
                $resp['status'] ='failed';
                $resp['sql'] =$sql;
            }
    ...
    function update_favorite(){
        extract($_POST);
        $sql = "UPDATE `favorite_list` set `quantity` = '{$quantity}' where `book_id` = '{$book_id}' and `user_id` = '{$user_id}'";
        $save = $this->query($sql);
        if($save){
            $resp['status'] ='success';
            $count = $this->query("SELECT SUM(quantity) as total FROM `favorite_list` where `user_id` = '{$user_id}'")->fetchArray()['total'];
            $resp['favorite_count'] = $count;
        }else{
            $resp['status'] ='failed';
            $resp['sql'] =$sql;
    ...
    function delete_from_favorite(){
        extract($_POST);
        $user_id = $_SESSION['user_id'];
        $sql = "DELETE FROM `favorite_list` where `book_id` = '{$id}' and `user_id` = '{$user_id}'";
    ...
    function book_to_borrow(){
        ...
        if($save){
            ...
            $favorite = $this->query("SELECT * FROM `favorite_list` where `user_id` = '{$user_id}'");
            while($row = $favorite->fetchArray()){
            ...
            $this->query("DELETE FROM `favorite_list` where `user_id` = '{$user_id}'");
            $_SESSION['flashdata']['type'] = 'success';
...
switch($a){
    ...
    case 'add_to_favorite':
        echo $action->add_to_favorite();
    break;
    case 'update_favorite':
        echo $action->update_favorite();
    break;
    case 'delete_from_favorite':
        echo $action->delete_from_favorite();
    break;
    case 'book_to_borrow':
