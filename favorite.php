        ...
        <div class="card">
            <div class="card-body">
                ...
                <table class="table table-bordered table-hover table-stripped">
                    ...
                    <tbody>
                        <?php 
                        $qry = $conn->query("SELECT c.*,b.title,b.description,b.isbn,b.author,b.sub_category_id FROM `favorite_list` c inner join `book_list` b on c.book_id = b.book_id where c.`user_id` = '{$_SESSION['user_id']}'");
                        $total = 0;
                        ...
                        <?php if(!$qry->fetchArray()): ?>
                            <tr>
                                <th class="text-center" colspan='2'>Your list is empty. <a href="./">Back to Home</a></th>
                            </tr>
                        <?php endif; ?>
...
<script>
    $(function(){
        ...
        $('.del_item').click(function(){
            _conf("Are you sure to remove this Book/Material from favorite list?",'delete_data',[$(this).attr('data-id')])
        })
    })
    function delete_data($id){
        $('#confirm_modal button').attr('disabled',true)
        $.ajax({
            url:'Actions.php?a=delete_from_favorite',
            method:'POST',
            data:{id:$id},
            dataType:'JSON',
            error:err=>{
            ...
            success:function(resp){
                if(resp.status == 'success'){
                    $('#favorite_count').text(resp.favorite_count)
                    calc()
